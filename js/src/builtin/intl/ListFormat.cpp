...
const ClassOps ListFormatObject::classOps_ = {nullptr, /* addProperty */
                                                nullptr, /* delProperty */
                                                nullptr, /* getProperty */
                                                nullptr, /* setProperty */
                                                nullptr, /* enumerate */
...
const Class ListFormatObject::class_ = {
    js_Object_str,
...
const Class& ListFormatObject::protoClass_ = PlainObject::class_;
...
static JSObject*
CreateListFormatPrototype(JSContext* cx, JSProtoKey key) {
...
void js::ListFormatObject::finalize(FreeOp* fop, JSObject* obj) {
...
bool js::AddListFormatConstructor(JSContext* cx, JS::Handle<JSObject*> intl) {
  JSObject* ctor = GlobalObject::createConstructor(cx, ListFormat, cx->names().ListFormat, 0);
  if (!ctor) {
    return false;
  }

  RootedValue ctorValue(cx, ObjectValue(*ctor));
  return DefineProperty(cx, intl, cx->names().ListFormat, ctorValue, 0);
}
...
static UListFormatter* NewUListFormatter(JSContext* cx,
  ...
  char* locale = bytes.encodeLatin1(cx, value.toString());
  if (!locale) {
    return nullptr;
  }
  ...
  }
  {
    ...
    if (StringEqualsAscii(strType, "conjunction")) {
      type = ListFormatType::Conjunction;
    } else if (StringEqualsAscii(strType, "disjunction")) {
      type = ListFormatType::Disjunction;
    } else {
      MOZ_ASSERT(StringEqualsAscii(strType, "unit"));
  ...
  }
  {
    ...
    if (StringEqualsAscii(strStyle, "long")) {
      style = ListFormatStyle::Long;
    } else if (StringEqualsAscii(strStyle, "short")) {
      style = ListFormatStyle::Short;
    } else {
      MOZ_ASSERT(StringEqualsAscii(strStyle, "narrow"));
  ...
  UListFormatter* lf = ulistfmt_open(IcuLocale(locale), &status);
  if (U_FAILURE(status)) {
