...
class PageProtectingVector final
{
    ...
    MOZ_ALWAYS_INLINE [[nodiscard]] bool reserveNewBuffer(size_t size) {
        unprotectOldBuffer();
    ...
    template<typename U>
    MOZ_ALWAYS_INLINE [[nodiscard]] bool appendNewPage(const U* values, size_t size) {
        size_t nextPage = uintptr_t(begin() + length() + size) >> pageShift;
        MOZ_ASSERT(currPage < nextPage);
        unprotectUnusedPartial(currPage, nextPage);
    ...
    template<typename U>
    MOZ_ALWAYS_INLINE [[nodiscard]] bool appendNewBuffer(const U* values, size_t size) {
        unprotectOldBuffer();
        bool ret = vector.append(values, size);
        protectNewBuffer();
        return ret;
    ...
    MOZ_NEVER_INLINE [[nodiscard]] bool reserveSlow(size_t size);
